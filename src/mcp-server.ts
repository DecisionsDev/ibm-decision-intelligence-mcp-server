...
// Helper function to validate a JSON schema object with proper $ref resolution
export function validateSchema(schema: OpenAPIV3_1.SchemaObject, components?: OpenAPIV3_1.ComponentsObject): boolean {
    try {
        // Use Ajv to validate schema completeness and $ref resolution
        // Configure custom logger to use our debug helper instead of console
        const ajv = new (Ajv as any)({
            strict: false,
            validateSchema: true,
            allErrors: true,
            logger: {
                log: (...args: any[]) => debug("Ajv log:", ...args),
                warn: (...args: any[]) => debug("Ajv warn:", ...args),
                error: (...args: any[]) => debug("Ajv error:", ...args)
            }
        });
        
        // If components are provided, add them as schemas for $ref resolution
        if (components?.schemas) {
            for (const [name, componentSchema] of Object.entries(components.schemas)) {
                ajv.addSchema(componentSchema, `#/components/schemas/${name}`);
            }
        }
        
        // Try to compile the schema - this will fail if there are unresolved $refs
        try {
            ajv.compile(schema);
        } catch (error) {
            debug("Schema validation failed:", String(error));
            return false;
        }
...
async function processOpenAPIPaths( configuration: Configuration, deploymentSpace: string, openapi: OpenAPIV3_1.Document, serviceId: string, toolNames: string[]): Promise<Omit<ToolDefinition, 'registeredTool'>[]> {
    ...
    for (const key in openapi.paths) {
        ...
        // Validate the schema generated by expandJSONSchemaDefinition
        ...
        if (inputSchema) {
            // Validate the schema before expansion, passing components for $ref resolution
            const schemas = openapi.components?.schemas;
            if (schemas && schemas.approval_input) {
                debug(`approval_input properties in components:`, Object.keys((schemas.approval_input as any).properties || {}));
            }
            const isValid = validateSchema(inputSchema, openapi.components);
            debug(`Schema validation for '${operationId}': ${isValid}`);
            if (!isValid) {
                debug(`Invalid schema for operation '${operationId}' in path '${key}'. Skipping this tool.`);
                continue;
            }
            
            const expandedSchema = expandJSONSchemaDefinition(inputSchema, schemas);
        }
